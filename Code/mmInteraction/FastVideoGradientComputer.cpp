...
//	_video->getFrame();
//	int channel = _video->getFrame()->nChannels;
...
//		cout << "H= " <<  _video->getFrame()->height << endl;
//		cout << "W= " <<  _video->getFrame()->width << endl;
//		cout << "FR= " <<  frame << endl;
//		cout << "CH= " <<  _video->getChannels() << endl;
//		cvShowImage("mat",_video->getFrame());
//		cvWaitKey(10);
		if (ok = _video->seek(static_cast<Video::FrameIndex>(frame))) {

//			if(_video->getChannels() != 0){
			cvResize(_video->getFrame(), _tmpFrame, CV_INTER_LINEAR);
