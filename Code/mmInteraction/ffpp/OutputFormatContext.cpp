...
{
	return AddStream(type, av_guess_codec(get()->oformat, NULL, get()->filename, NULL, (AVMediaType)type));
}

Stream OutputFormatContext::AddStream(enum AVMediaType type, enum CodecID codec_id)
{
	AVStream *pStream = av_new_stream(get(), get()->nb_streams);
	if (!pStream) throw(std::runtime_error("Could not create the output stream"));
	
	avcodec_get_context_defaults2(pStream->codec, (AVMediaType)type);
