...
Video::Video(const std::string& fileName, double sX, double sY){
	_fileName = fileName;
	_currentBuffer = 0;
	_peekBuffer = 0;
	_startPts = 0;
...
void Video::init()
{
	...
	*_inCodecCtx = _inStream->GetCodecContext();
	...
	_width = _inCodecCtx->GetWidth()*_scaleX;
	_height = _inCodecCtx->GetHeight()*_scaleY;
//	_channels = _inCodecCtx->GetChannels();
	...
	if (DEBUGOUT) std::cout << "# height: " << _height << std::endl;
	if (DEBUGOUT) std::cout << "# width: " << _width << std::endl;
...
void Video::setScaleX(double sX){
	_scaleX = sX;
}

void Video::setScaleY(double sY){
	_scaleY = sY;
}
...
bool Video::advance(double seekToSec, bool force)
{
	...
	while (!(foundFrame && _currentBuffer) && (optInPacket = _inFormatCtx->ReadPacket())) {
		...
		if (DEBUGOUT) std::cout << "# packet size: " << inPacket.GetSize() << std::endl;
		// try to decode frame
		...
		if (DEBUGOUT) std::cout << "# height: " << inFrame.getHeight() << std::endl;
		if (DEBUGOUT) std::cout << "# width: " << inFrame.getWidth() << std::endl;
