...
public:
	Video(const std::string& fileName, double sX, double sY);
	/*: _fileName(fileName), _currentBuffer(0), _peekBuffer(0),
		_startPts(0), _currentPts(0), _peekPts(0),
		_frameRate(0), _invFrameRate(0),
		_streamTimeBase(0), _width(0), _height(0), _channels(0), _probedDuration(0),
		_inStream(0), _inCodecCtx(0), _inFormatCtx(0), _scalerCtx(0), _scaleX(sX), _scaleY(sY)
	{
		init();
	}
*/
