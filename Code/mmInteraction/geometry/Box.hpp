...
template<typename T>
inline void Box<T>::setTopLeft(const PointGeometry<T> &p) { setLeft(p.getX()); setTop(p.getY()); }

template<typename T>
inline void Box<T>::setTopRight(const PointGeometry<T> &p) { setRight(p.getX()); setTop(p.getY()); }

template<typename T>
inline void Box<T>::setBottomLeft(const PointGeometry<T> &p) { setLeft(p.getX()); setBottom(p.getY()); }

template<typename T>
inline void Box<T>::setBottomRight(const PointGeometry<T> &p) { setRight(p.getX()); setBottom(p.getY()); }

template<typename T>
inline PointGeometry<T> Box<T>::getCenter() const
{ return PointGeometry<T>(xp + w/2, yp + h/2); }
...
template<typename T>
inline void Box<T>::moveTopLeft(const PointGeometry<T> &p) { moveLeft(p.getX()); moveTop(p.getY()); }

template<typename T>
inline void Box<T>::moveTopRight(const PointGeometry<T> &p) { moveRight(p.getX()); moveTop(p.getY()); }

template<typename T>
inline void Box<T>::moveBottomLeft(const PointGeometry<T> &p) { moveLeft(p.getX()); moveBottom(p.getY()); }

template<typename T>
inline void Box<T>::moveBottomRight(const PointGeometry<T> &p) { moveRight(p.getX()); moveBottom(p.getY()); }

template<typename T>
inline void Box<T>::moveCenter(const PointGeometry<T> &p) { xp = p.getX() - w/2; yp = p.getY() - h/2; }
...
template<typename T>
inline SizeGeometry<T> Box<T>::getSize() const
{ return SizeGeometry<T>(w, h); }
...
template<typename T>
inline void Box<T>::translate(const PointGeometry<T> &p)
{
	xp += p.getX();
	yp += p.getY();
}
...
template<typename T>
inline void Box<T>::moveTo(const PointGeometry<T> &p)
{
	xp = p.getX();
	yp = p.getY();
}
...
template<typename T>
inline Box<T> Box<T>::translated(const PointGeometry<T> &p) const
...
template<typename T>
inline void Box<T>::setSize(const SizeGeometry<T> &s)
{
	w = s.getWidth();
	h = s.getHeight();
}
...
{
	return contains(PointGeometry<T2>(ax, ay));
}
...
template<typename T>
template<typename T2>
bool Box<T>::contains(const PointGeometry<T2> &p) const
{
	if (isEmpty())
		return false;
	Box r = normalized();
...
{
	PointGeometry<T> topLeft;
	SizeGeometry<T> SizeGeometry;
	i.setf(std::ios_base::skipws);
	i >> topLeft;
	i.ignore(256, ',');
	i >> SizeGeometry;
	b = Box<T>(topLeft, SizeGeometry);
	return i;
}
